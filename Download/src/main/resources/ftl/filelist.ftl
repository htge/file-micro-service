...
<html>
	<head>
		...
		<#if title?exists><title>${title}文件夹</title>
		<#else><title>根目录</title>
		</#if><style>
	...
	<body>
	<h1><#if title?exists>
		${title}文件夹&nbsp;<a style="color: white; text-decoration: none" href="../">返回到${back}/</a>
	<#else>根目录<#if settingPath?exists>&nbsp;<a style="color: white; text-decoration: none" href="${settingPath}">设置</a></#if>
	</#if></h1>
	<hr style="height: 1px;">
